...
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        printf("No se ingresaron la cantidad minima de archivos requeridos :\n");
        return 1;
    }
    if (getInfo(argv[1], &info_inicial) != 0)
        return 2;

    //printf("EL QUE EMPIEZA JUGANDO ES: %d",info_inicial.whoStart);

    DataForEndGame info_final = SimularJuego(info_inicial);
    /* printf("LA CANTIDAD DE MOVIMIENTOS SON %d\n",info_inicial.movidas.tamanio);
    printf("la primera columna es:%d\n",info_inicial.movidas.celdas[0].columna);
    printf("la primera fila es:%d\n",info_inicial.movidas.celdas[0].fila);
    printf("la segunda columna es:%d\n",info_inicial.movidas.celdas[1].columna);
    printf("la segunda fila es:%d\n",info_inicial.movidas.celdas[1].fila);
    printf("la tercer columna es:%d\n",info_inicial.movidas.celdas[2].columna);
    printf("la tercer fila es:%d\n",info_inicial.movidas.celdas[2].fila);
    */
   // printf("EL QUE ULTIMO JUGO FUERON LOS : %d",info_final.WhoendGame);
    //printf("EL ESTADO FINAL ES: %d",info_final.status);
    mostrarMensajeSituacion(info_inicial.players, info_final);
    ...
    if (info_final.status == 1){
        //printf("entroooooooo");
       // printf("EL QUE ULTIMO JUGO FUERON LOS : %d",info_final.WhoendGame);
        code = writeTableroinFile(argv[2], info_final);

}
    freeListadoDeCeldas(&(info_inicial.movidas));
